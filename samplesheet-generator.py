...
def main():
    """Main function to parse arguments and generate samplesheet"""
    parser = argparse.ArgumentParser(description="Generate samplesheet for nf-core pipelines.")
    parser.add_argument("-m", "--mode", required=True, choices=PIPELINE_CONFIG.keys(), help="Pipeline mode (e.g., sarek, oncoanalyser, rnaseq, viralcon)")
    parser.add_argument("-i", "--input", required=True, help="Path to the directory containing FastQ files")
    parser.add_argument("-o", "--output", default="output", help="Output directory for the samplesheet (default: 'output')")
    
    args = parser.parse_args()
    
    # Generate samplesheet
    generate_samplesheet(args.mode, args.input, args.output)
